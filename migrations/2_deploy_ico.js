...
module.exports = async function(deployer, network, accounts) {
  ...
  //       uint256 _cap
    const rate = 75000; // 1 eth = 1000 WRT tokens
    const initialRate = rate;
    const finalRate = 50000;
    const wallet = accounts[0];
    const timeNow = Math.floor(Date.now() / 1000);
    const openingTime = timeNow  + duration.seconds(30);
    const closingTime = timeNow  + duration.days(1);
    const cap = web3.toWei(1); // 100 eth

    await deployer.deploy(WeatherCrowdsale, rate, wallet, deployedToken.address, openingTime, closingTime, initialRate, finalRate, cap);
